...
q0 = rand(len(halfSittingConfig[robotName]))
q0=vectorToTuple(q0)
robot.set(q0)
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>-
...
gotoNd(contactLF,(0,0,0.105),'111111',10)
push(contactLF)
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>-
...
erh = array(taskRH.task.error.value)
print "Task residue is: ||e|| = "+str(linalg.norm(erh))
