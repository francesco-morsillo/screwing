...
"""
x0=0.48121218182012693
y0=0.79866091958569707
z0=0.9850812953368490
roll=0.46018864580232682
pitch=1.0619713142838436
yaw=2.207615023381436
"""
...
roll=random.uniform(0,pi/4)
pitch=random.uniform(0,pi/4)
yaw=random.uniform(-pi,pi/2)
...
LFMatH = contactLF.feature.position.value
LFx = LFMatH[0][3]
LFy = LFMatH[1][3]
#LFz = 0.105
RFMatH = contactRF.feature.position.value
RFx = RFMatH[0][3]
RFy = RFMatH[1][3]
#RFz = 0.105

LFPos = vectorToTuple(array([LFx,LFy,0.]))

robot.viewer.updateElementConfig('zmp',LFPos+(0,0,0))

aimCOM = vectorToTuple(array([(LFx+RFx)/2,(LFy+RFy)/2,0.]))	# z not important
taskCom.ref = aimCOM
taskCom.feature.selec.value = '011'
taskCom.task.controlGain.value = 10

# Set the aims
contactLF.ref = LFMatH
contactRF.ref = RFMatH
push(taskCom)
...
eg = array(dyn.dynamicDrift.value)
print "Task residue is: ||e|| = "+str(linalg.norm(eg))
